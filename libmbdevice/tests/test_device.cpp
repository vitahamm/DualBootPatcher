...
TEST(DeviceTest, CheckDefaultValues)
{
    ...
    ASSERT_EQ(device.tw_flags(), 0);
    ASSERT_EQ(device.tw_pixel_format(), mb::device::TwPixelFormat::DEFAULT);
    ASSERT_EQ(device.tw_force_pixel_format(), mb::device::TwForcePixelFormat::NONE);
...
TEST(DeviceTest, CheckGettersSetters)
{
    ...
    device.set_architecture(mb::device::ARCH_ARM64_V8A);
    ASSERT_EQ(device.architecture(), mb::device::ARCH_ARM64_V8A);

    uint64_t device_flags = mb::device::DeviceFlag::HAS_COMBINED_BOOT_AND_RECOVERY;
