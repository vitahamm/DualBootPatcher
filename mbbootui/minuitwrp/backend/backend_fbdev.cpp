...
extern "C" struct minui_backend * BACKEND_FUNCTION(fbdev)()
{
    return &my_backend;
}
...
static void fbdev_blank(minui_backend* backend __unused, bool blank)
{
    ...
    if ((tw_device.tw_flags() & mb::device::TwFlag::TW_NO_SCREEN_BLANK)
    ...
    } else if (!(tw_device.tw_flags() & mb::device::TwFlag::TW_NO_SCREEN_BLANK)) {
        int ret;
...
static GRSurface* fbdev_init(minui_backend* backend)
{
    ...
    if (tw_device.tw_flags() & mb::device::TwFlag::TW_GRAPHICS_FORCE_USE_LINELENGTH) {
    ...
    if (tw_device.tw_pixel_format() == mb::device::TwPixelFormat::BGRA_8888) {
...
static GRSurface* fbdev_flip(minui_backend* backend __unused)
{
    ...
    if (!(tw_device.tw_flags() & mb::device::TwFlag::TW_BOARD_HAS_FLIPPED_SCREEN)) {
        if (double_buffered) {
