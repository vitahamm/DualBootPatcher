...
static struct timespec lastInputStat;
...
static int vk_init(struct ev *e)
{
    ...
    if (!input_whitelist.empty()) {
        ...
        if (strcmp(e->deviceName, "bma250") == 0
                ...
                // Galaxy S4 temperature/humidity sensor sends events every few
                // seconds, which prevents screen blanking from happening
                || strcmp(e->deviceName, "temp_humidity_sensor") == 0) {
    ...
    for (i = 0; i < e->vk_count; ++i) {
        ...
        for (j = 0; j < 6; ++j) {
            token[j] = vk_strtok_r((i || j) ? nullptr : vks, ":", &ts);
        }
        ...
        e->vks[i].scancode = strtol(token[1], nullptr, 0);
        e->vks[i].centerx = strtol(token[2], nullptr, 0);
        e->vks[i].centery = strtol(token[3], nullptr, 0);
        e->vks[i].width = strtol(token[4], nullptr, 0);
        e->vks[i].height = strtol(token[5], nullptr, 0);
    }
...
static int vk_tp_to_screen(struct position *p, int *x, int *y)
{
    ...
    int fb_width;
    int fb_height;
    if (!(tw_device.tw_flags() & mb::device::TwFlag::TW_TOUCHSCREEN_SWAP_XY)) {
...
static int vk_modify(struct ev *e, struct input_event *ev)
{
    ...
    if (ev->type == EV_ABS) {
        ...
        case ABS_MT_POSITION: //2a
            e->mt_p.synced = 0x03;
            if (ev->value == (1 << 31)) {
                if (!(tw_device.tw_flags() & mb::device::TwFlag::TW_IGNORE_MT_POSITION_0)) {
                    e->mt_p.x = 0;
                    e->mt_p.y = 0;
                    lastWasSynReport = 1;
                }
        ...
        case ABS_MT_TOUCH_MAJOR: //30
            if (ev->value == 0) {
                if (!(tw_device.tw_flags() & mb::device::TwFlag::TW_IGNORE_MAJOR_AXIS_0)) {
        ...
        case ABS_MT_TRACKING_ID: //39
            if (tw_device.tw_flags() & mb::device::TwFlag::TW_IGNORE_ABS_MT_TRACKING_ID) {
#ifdef _EVENT_LOGGING
    ...
    if (((lastWasSynReport == 1 || touchReleaseOnNextSynReport == 1)
                && !use_tracking_id_negative_as_touch_release)
            || (use_tracking_id_negative_as_touch_release
                && touchReleaseOnNextSynReport == 2)) {
        // Reset the value
    ...
    if (tw_device.tw_flags() & mb::device::TwFlag::TW_TOUCHSCREEN_FLIP_X) {
        x = gr_fb_width() - x;
    }
    if (tw_device.tw_flags() & mb::device::TwFlag::TW_TOUCHSCREEN_FLIP_Y) {
        y = gr_fb_height() - y;
    }
