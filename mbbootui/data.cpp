...
int DataManager::LoadValues(const std::string& filename)
{
    ...
    if (!(tw_device.tw_flags() & mb::device::TwFlag::TW_NO_SCREEN_TIMEOUT)) {
...
void DataManager::SetDefaultValues()
{
    ...
    if (tw_device.tw_flags() & mb::device::TwFlag::TW_HAS_DOWNLOAD_MODE) {
    ...
    if (tw_device.tw_flags() & mb::device::TwFlag::TW_NO_CPU_TEMP) {
    ...
    if (findbright.empty()) {
        ...
        findbright = Find_File::Find(
                "brightness",
                (tw_device.tw_flags() & mb::device::TwFlag::TW_PREFER_LCD_BACKLIGHT)
